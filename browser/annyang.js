...
(function (undefined) {
  ...
  var registerCommand = function(command, cb, phrase) {
    commandsList.push({ command: command, callback: cb, originalPhrase: phrase, count: 0 });
    if (debugState) {
  ...
  root.annyang = {
        ...
        for (var i = 0; i<results.length; i++) {
          ...
          for (var j = 0, l = commandsList.length; j < l; j++) {
            ...
            var currentCommand = commandsList[j];
             var c = words.indexOf( commandsList[j].originalPhrase);
              //var result = currentCommand.command.exec(words[c]);

            if (c > -1) {
              loop3:
              for (var d = 0; d < words.length; d++){
                if (words[d] === commandsList[j].originalPhrase){
                  commandsList[j].count++;
                  root.updateCount();
                  root.update(words[d], commandsList[j].count);
                }
              }
              totalText += commandText + ' ';
              found = true;
              //break loop1;
            }
            ...
            //   return true;
            // }
            else if (i === results.length -1 && !found) {
              totalText += results[0] + ' ';
              break;
            }
          }
     ...
     * @see [Commands Object](#commands-object)
     */
    addCommands: function(name, command) {
      var cb;
      initIfNeeded();
      if (typeof command === 'function') {
        registerCommand(commandToRegExp(name), command, name);
 ...
 * * Spanish sole.og(Costa Rica) `es-CR`
